...
function getGoDocstring(node: TSNode, source: string): string | undefined {
  ...
  if (docs.length === 0 && node.parent) {
    cur = node.parent.previousNamedSibling;
    while (cur && cur.type === "comment") {
      const text = getNodeText(cur, source).trim();
      if (text.startsWith("//")) {
        docs.unshift(text.slice(2).trim());
...
class GoWalker {
  ...
  private readonly deferredMethods: { node: TSNode; stack: string[] }[] = [];
  ...
  run(root: TSNode): { nodes: ExtractedNode[]; edges: ExtractedEdge[] } {
    ...
    // Pass 1: extract everything except methods, which are deferred so a
    // method can bind to a receiver type declared later in the same file.
    for (const child of root.namedChildren) {
      if (child.type === "method_declaration") {
        this.deferredMethods.push({ node: child, stack: [...this.scopeStack] });
      } else {
        this.visit(child);
      }
    }

    // Pass 2: extract all methods, binding them to their receiver types.
    for (const { node, stack } of this.deferredMethods) {
  ...
  private extractMethod(node: TSNode): void {
    ...
    // Go methods live outside their type in the source. When the receiver type
    // is in this file, the method's containment parent and qualified name come
    // from that type (single containment chain, receiver-stable identity). When
    // it is not (another file / an unknown type), fall back to the file scope.
    const receiverNode = receiverType
      ? this.nodes.find(
          (n) =>
            n.name === receiverType &&
      ...
      : undefined;
    if (receiverNode) this.scopeStack.push(receiverNode.id);
    ...
    if (receiverNode) this.scopeStack.pop();
    if (!id) return;
  ...
  private extractVarSpec(node: TSNode): void {
    ...
    for (const nameNode of node.namedChildren) {
      ...
      this.createNode("variable", name, node, {
        signature: valueNode
          ? getNodeText(valueNode, this.source).slice(0, 200)
          : typeNode
...
function typeParametersOf(node: TSNode, source: string): string[] | undefined {
  ...
  for (let i = 0; i < typeParams.namedChildCount; i++) {
    ...
    if (!child) continue;
    // `type_parameter_declaration` (tree-sitter-go v0.25) / `parameter_declaration`
    // (older prebuilt wasm). A single declaration can bind several names to one
    // constraint: `T, U any`.
    if (
