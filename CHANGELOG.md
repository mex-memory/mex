...
### Fixed
- `mex impact` no longer drops grounding records first when the output budget runs out. They are admitted right after the target's definitions, ahead of transitive callers and source ranges, so a target with many callers still returns its knowledge links at the default budget; the response is still marked `truncated` when callers are cut (#225).
