...
### Changed
- `mex graph refresh` now does work in proportion to what changed. A tree-sitter file is re-extracted only when its bytes changed; a TypeScript or JavaScript file when it changed or imports a changed file, directly or through re-exports; every other file reuses its cached extraction, and resolution still runs over the whole corpus, so the graph is the one a full refresh would publish. Only the files whose stored rows changed are rewritten, and a refresh with nothing to publish, not even coverage or HEAD, now leaves `graph.db` untouched instead of copying, validating and republishing it. After publication the live file is checked to be the validated candidate by identity and SHA-256 instead of being inspected a second time. A maintenance copy of `graph.db` is hashed from the same read that writes it, and a rollback copy is fsynced only when it is restored or retained as recovery. A change that can reach files without an import (a script, a `.d.ts`, a global or module augmentation), any change to a `package.json` or `tsconfig`, a changed non-corpus compiler input, or a change affecting more than 70% of the compiler files re-extracts everything, and the JSON result now reports `refresh.mode`, the fallback reason and the files changed, re-extracted and rewritten. A refresh after commits that touched no indexed file now records the new HEAD; `indexedHead` used to stay behind while status read fresh. The accepted cost: the per-file cache adds to `graph.db`. **One-time full extraction:** rendered TypeScript types no longer depend on the order the compiler happened to check files in, which a partial re-extraction cannot reproduce: union and intersection members and the members of object types are listed in one canonical order, and a call through a union-typed callee records every member's signature rather than the one the compiler built from its first member. A callee whose value is one of several alternatives (`a ?? b`, `a || b`, `c ? a : b`, or an element of an array literal reached through `for...of`, an element access or an array-method callback, directly or through an unannotated `const`) is treated the same way, since the compiler collapses identical alternatives to whichever it saw first and the call's target used to depend on that. The TypeScript extractor version is now `typescript-5.9-v5`, so the first refresh after upgrading re-extracts the whole corpus, and a signature that contained such a type changes, together with its node id; refresh aliases each old id to the new one as for #240 (#209).
...
### Fixed
- Renaming a small function no longer reads as a deletion. A node under 30 normalized tokens, such as a one- to three-line wrapper, was reported `GROUNDING_GONE` after any rename, because its body is too small to tell apart from other wrappers and the reconciler gave up before looking. Small nodes are now matched by their callers and callees, which a rename leaves in place: one same-kind node sharing at least two of them with neighbor Jaccard of at least 0.8 and a body of the same shape is `MOVED`; several such nodes, or a node that shares the callers but not the shape, or one whose committed neighbors have gone stale since it was grounded, read `GROUNDING_AMBIGUOUS` naming the candidate; no match stays `GONE`. A function deleted in favour of an existing one that its callers switched to is not moved to it. Reconciliation also uses the committed `bodyHash` to settle a tie between equally good matches when exactly one has the grounded text, which is what a node whose id changed without its source changing looks like. Normal-sized nodes return the verdicts they did before. Reconciling the 495 old ids from #240 on a fresh Hono clone, 240 now move to the right node (was 158), none to a wrong one, 164 read `AMBIGUOUS` and 55 `GONE` (were 128 and 173). The accepted costs: a small function that was deleted while another function of the same shape still has its callers now reads `AMBIGUOUS` rather than `GONE`, and a replacement that had no callers of its own before it took over, with the same shape and callees, is indistinguishable from a rename and is moved. That move is not accepted silently: every `MOVED` decided by callers and callees rather than body similarity is reported as the new info-level `GROUNDING_MOVED_BY_NEIGHBORS`, naming old → new, in both `mex check` and `mex sync` output, and it is not counted in the drift score (#229).
- An inline `mex://` anchor whose node is gone now reconciles from the committed fingerprint, as the `grounds_to` entry for the same node does. It looked only at the current graph and the local cache, so on a fresh build or after `graph rebuild` it read `GROUNDING_GONE` ("unavailable node") while the entry for the same node in the same run reconciled `MOVED` or `AMBIGUOUS`. The anchor's baseline is now the current graph, then the same file's `grounds_to` entry, then an entry for the node in any other scaffold file (read through the contained, bounded reader `impact` uses), then the cache; `mex check` and `mex sync` read it the same way. Entries in other files that disagree about the fingerprint read `GROUNDING_AMBIGUOUS` rather than one being chosen. The accepted cost: an anchor with no entry in its own file may read the scaffold once more during `check` (#229).
- TypeScript node ids no longer depend on the directory a repository is checked out in. The compiler printed a type from another module as `import("<absolute path>").Name`, and the signature is part of a node's identity. So two clean builds of `honojs/hono` in different directories disagreed on 493 of 13,628 node ids and 3,247 edges, and `graph get`, `graph query` and `impact` showed the checkout path, user name included. Module references in signatures now render repo-relative (`import("./src/jsx/base")`), package-relative under `node_modules`, and as `<external>/<file name>` anywhere else. Signatures are also no longer truncated, because the compiler chose its truncation point from the text with the path still in it. The same two builds now agree on every id and edge, and no stored text contains the checkout path. The TypeScript extractor version is now `typescript-5.9-v3`, so an existing graph reads stale until `mex graph refresh` rebuilds it. **One-time id change for TypeScript groundings:** a `grounds_to` entry or `mex://` anchor on an affected symbol holds its old id. Where the graph was built before the upgrade, refresh aliases each old id to the new one, and `mex sync` rewrites the scaffold; on Hono that covered every affected symbol outside test files. **After upgrading, run `mex graph refresh` and then `mex sync` once on a checkout that still has its old graph, and commit the scaffold,** so teammates and CI never see the old ids. A fresh clone has no old graph to alias from, so the grounding reconciles by fingerprint instead. A confident match is `MOVED`, and `mex sync` rewrites it. A short function, a node without a fingerprint, or a node whose neighbours were also affected reads `GROUNDING_AMBIGUOUS` or `GROUNDING_GONE` until it is re-grounded. The accepted cost: the rewrite shows up once as an id change in scaffold diffs. Untruncated signatures made the Hono graph 6% larger, and its largest signature grew from 36 KB to 197 KB; agent output still caps signatures (#240).
- Grounding reconciliation no longer rebinds a missing node to one of two equally good matches. When a grounded body has an exact or near copy elsewhere, both candidates scored alike and the tie went to whichever id sorted first, so `mex sync` could silently move a grounding to the copy. A `MOVED` match must now lead the runner-up by 0.08, the same margin refresh's fingerprint aliases use; otherwise it reads `GROUNDING_AMBIGUOUS` with the best candidate. Reconciling the 495 old ids from #240 on a fresh Hono clone, 14 used to move to the wrong node and now none do. The accepted cost: 113 correct moves, all of them test-file callbacks with near-identical siblings, also become `AMBIGUOUS` and wait for review instead of being rewritten (#240).
...
### Added
- A stack section's architectural vocabulary no longer produces dependency warnings. `mex check` skips a claim that names a part of a system rather than a package — labels such as **Frontend**, **Middleware** or **Observability**, and concept acronyms such as **SPA**, **CRUD**, **MVC** or **SSR** — alongside the runtimes and platforms it already skipped. The rule is applied in the dependency checker, where the manifest is known, so a project that genuinely declares a package by one of those names keeps it checked, and a capitalized package spelling carrying separators (`PINO-HTTP`, `GRAPHQL-WS`, `@SCOPE/PKG`) is unaffected. The accepted cost: a package whose name is short enough to be written in capitals (`cors`, `ajv`) is no longer reported once it disappears from the manifest, because nothing in the name separates that from an acronym; version claims on such a name are still compared (#4).
- A bounded FastAPI route resolver connecting `@app.get()`, `@router.post()` and the other HTTP shortcut decorators — plus `@app.api_route()` with an explicit `methods=` list — to their handler functions. One route node is emitted per declared method, with an ordinal in the role so a path declared twice in one module cannot collide ids and fail the build. FastAPI path parameters (`/items/{item_id}`, `/files/{file_path:path}`) are preserved verbatim, an `APIRouter` constructed in the same file composes its static `prefix`, decorators split across lines are read as one, comments and docstrings are blanked before scanning, receivers include routers imported from another module, non-static paths (f-strings, `%`-format) are skipped rather than guessed, same-file handlers resolve only when unambiguous, and edges are labeled `fastapi-route-handler`. Detection keys on a staged Python module importing fastapi — the reliable observable, since dependency manifests are not staged corpus files (#111).
...
### Fixed
- `mex impact` no longer drops grounding records first when the output budget runs out. They are admitted right after the target's definitions, ahead of transitive callers and source ranges, so a target with many callers still returns its knowledge links at the default budget; the response is still marked `truncated` when callers are cut. The accepted cost: on a target with many groundings, a few caller or source records that used to fit now give way to them (#225).
